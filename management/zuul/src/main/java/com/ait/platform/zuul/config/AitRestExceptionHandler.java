...
public class AitRestExceptionHandler extends ResponseEntityExceptionHandler {
	...
	@ResponseBody
	public ResponseEntity<Object> handleFileException(Exception ex, final WebRequest request) {
		ex.printStackTrace();
		final AitException AitException = new AitException(HttpStatus.BAD_REQUEST, ex.getLocalizedMessage(), "Error al cargar la información. Verifique el tamaño del archivo");
		// return new ResponseEntity<Object>(AitException, new HttpHeaders(), AitException.getStatus());
		return handleExceptionInternal(ex, AitException, new HttpHeaders(), AitException.getStatus(), request);
	}
}